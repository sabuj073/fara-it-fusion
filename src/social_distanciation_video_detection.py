...
while True:	
	...
	img = cv2.imread("../img/black_background.png")
	bird_view_img = img
	...
	if count_red>1:
		...
		cv2.putText(frame, 'People at Risk: ', (x + int(w/10),y + int(h/2)), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 255), 1)
		
	# cv2.putText(frame, 'People at Risk: '+str(count_red), (5, 30), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 255), 1)
	cv2.imshow("Bird view", bird_view_img)
	cv2.imshow("Fara It Fusion", frame)
	...
	if output_video_1 is None and output_video_2 is None:
		...
		output_video_1 = cv2.VideoWriter("../output/video_custom.avi", fourcc1, 25,(frame.shape[1], frame.shape[0]), True)
